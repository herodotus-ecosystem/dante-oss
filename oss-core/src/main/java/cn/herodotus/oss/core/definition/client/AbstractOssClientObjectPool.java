...
package cn.herodotus.oss.core.definition.client;

import cn.herodotus.oss.core.definition.properties.AbstractOssProperties;
import cn.herodotus.oss.core.exception.OssClientPoolErrorException;
...
/**
 * <p>Description: 对象存储 Client 对象池抽象定义 </p>
 *
 * @author : gengwei.zheng
 * @date : 2023/7/23 12:22
 */
public abstract class AbstractOssClientObjectPool<T> {
    ...
    public AbstractOssClientObjectPool(AbstractOssClientPooledObjectFactory<T> factory) {
        ...
        config.setLifo(properties.getPool().getLifo());
        config.setBlockWhenExhausted(properties.getPool().getBlockWhenExhausted());
        genericObjectPool = new GenericObjectPool<>(factory, config);
    }

    public T get() {
        try {
            T client = genericObjectPool.borrowObject();
