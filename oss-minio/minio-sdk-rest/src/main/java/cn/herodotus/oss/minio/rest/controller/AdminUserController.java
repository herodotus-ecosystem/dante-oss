...
import cn.herodotus.oss.minio.core.converter.UserInfoToDomainConverter;
import cn.herodotus.oss.minio.core.converter.UsersToDomainsConverter;
import cn.herodotus.oss.minio.core.domain.UserDomain;
import cn.herodotus.oss.minio.logic.service.AdminUserService;
...
public class AdminUserController implements Controller {
    ...
    public Result<List<UserDomain>> list() {
        Map<String, UserInfo> users = adminUserService.listUsers();
    ...
    public Result<UserDomain> get(String accessKey) {
        UserInfo userInfo = adminUserService.getUserInfo(accessKey);
    ...
    public Result<Boolean> add(@Validated @RequestBody UserDomain domain) {
        adminUserService.addUser(domain.getAccessKey(), UserInfo.Status.fromString(domain.getStatus().name()), domain.getSecretKey(), domain.getPolicyName(), domain.getMemberOf());
        return result(true);
    }
    ...
    public Result<Boolean> remove(String accessKey) {
        adminUserService.deleteUser(accessKey);
        return result(true);
    }
}
