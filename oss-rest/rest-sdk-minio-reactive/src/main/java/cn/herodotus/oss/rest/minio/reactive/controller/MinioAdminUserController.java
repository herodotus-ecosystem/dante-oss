...
import cn.herodotus.oss.dialect.reactive.minio.service.MinioAdminUserService;
...
public class MinioAdminUserController implements OssController {
    ...
    @GetMapping("/list")
    public Mono<Result<List<UserDomain>>> list() {
        Mono<Map<String, UserInfo>> users = minioAdminUserService.listUsers();
        return result(users.mapNotNull(toDomains::convert));
    }
    ...
    @GetMapping
    public Mono<Result<UserDomain>> get(String accessKey) {
        Mono<UserInfo> userInfo = minioAdminUserService.getUserInfo(accessKey);
        return result(userInfo.mapNotNull(toDomain::convert));
    }
    ...
    @PostMapping
    public Mono<Result<VoidResponse>> add(@Validated @RequestBody UserDomain domain) {
        Mono<Void> result = minioAdminUserService.addUser(domain.getAccessKey(), UserInfo.Status.fromString(domain.getStatus().name()), domain.getSecretKey(), domain.getPolicyName(), domain.getMemberOf());
    ...
    @DeleteMapping
    public Mono<Result<VoidResponse>> remove(String accessKey) {
        Mono<Void> result = minioAdminUserService.deleteUser(accessKey);
        return fromVoid(result);
    }
}
