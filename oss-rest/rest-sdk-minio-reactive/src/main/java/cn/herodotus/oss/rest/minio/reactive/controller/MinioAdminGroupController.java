...
import cn.herodotus.oss.dialect.reactive.minio.service.MinioAdminGroupService;
...
public class MinioAdminGroupController implements OssController {
    ...
    @GetMapping("/list")
    public Mono<Result<List<String>>> list() {
        Mono<List<String>> groups = minioAdminGroupService.listGroups();
    ...
    @Operation(summary = "获取组信息", description = "获取 Minio 组信息",
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(mediaType = "application/json")),
            responses = {
                    @ApiResponse(description = "组信息", content = @Content(mediaType = "application/json")),
    ...
    @GetMapping
    public Mono<Result<GroupDomain>> get(String group) {
        Mono<GroupInfo> groupInfo = minioAdminGroupService.getGroupInfo(group);
        return result(groupInfo.mapNotNull(toDomain::convert));
    }
    ...
    @Parameters({
            @Parameter(name = "domain", required = true, description = "GroupDomain实体", schema = @Schema(implementation = GroupDomain.class))
    })
    @PostMapping
    public Mono<Result<VoidResponse>> add(@Validated @RequestBody GroupDomain domain) {
        Mono<Void> result = minioAdminGroupService.addUpdateGroup(domain.getName(), domain.getStatus(), domain.getMembers());
    ...
    @DeleteMapping
    public Mono<Result<VoidResponse>> remove(String group) {
        Mono<Void> result = minioAdminGroupService.removeGroup(group);
        return fromVoid(result);
    }
}
