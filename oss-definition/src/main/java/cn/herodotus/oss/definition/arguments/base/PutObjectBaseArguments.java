...
public abstract class PutObjectBaseArguments extends ObjectWriteArguments {
    ...
    @Max(value = MAX_OBJECT_SIZE, message = "对象允许的最大 Size 为 5TiB")
    protected Long objectSize;

    @Schema(name = "分片的大小", description = "分片的大小只能 >= 5M 同时 <= 5G")
    @Min(value = MIN_MULTIPART_SIZE, message = "分片最小Size不能小于 5MiB")
    @Max(value = MAX_PART_SIZE, message = "分片最小Size不能超过 is 5GiB ")
